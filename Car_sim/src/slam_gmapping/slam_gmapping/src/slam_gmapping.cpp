...
bool SlamGmapping::initMapper(const sensor_msgs::msg::LaserScan::ConstSharedPtr scan)
{
    ...
    // MODIFIED: Force initial pose to (0, 0, 0) regardless of current odometry
    // This ensures map and odom start aligned
    GMapping::OrientedPoint initialPose(0.0, 0.0, 0.0);
    RCLCPP_INFO(this->get_logger(), "Initializing SLAM at pose (0, 0, 0) - map and odom aligned at start");
